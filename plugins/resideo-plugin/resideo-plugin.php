...
/* code to prevent plugin updates */
function resideo_plugin_updates_disable( $value ) {
    if( isset( $value->response['resideo-plugin/resideo-plugin.php'] ) ) { 
    unset( $value->response['resideo-plugin/resideo-plugin.php'] );
    }
    return $value;
    }
add_filter( 'site_transient_update_plugins', 'resideo_plugin_updates_disable' );
...
add_shortcode('sc_show_contact_page','sc_show_contact_page_fn');
