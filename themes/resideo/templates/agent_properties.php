            ...
            <div class="col-sm-12 col-md-6 col-lg-4">
                <a href="<?php echo esc_url($s_link); ?>" class="pxp-prop-card-1 rounded-lg mb-4">
                    ...
                    <div class="pxp-prop-card-1-details">
                        <div class="pxp-prop-card-1-details-title"><?php echo pll_($s_title); ?></div>
                        <div class="pxp-prop-card-1-details-price">
                            <?php if ($currency_pos == 'before') {
                                echo pll_($currency_val) . pll_($s_price) . ' <span>' . pll_($s_price_label) . '</span>';
                            } else {
                                echo pll_($s_price) . pll_($currency_val) . ' <span>' . pll_($s_price_label) . '</span>';
                            } ?>
                        </div>
                        <div class="pxp-prop-card-1-details-features text-uppercase ">
                            <?php if ($s_beds != '') {
                                echo pll_($s_beds) . ' ' . pll_($beds_label) . '<span>|</span>';
                            }
                            if ($s_baths != '') {
                                echo pll_($s_baths) . ' ' . pll_($baths_label) . '<span>|</span>';
                            }
                            if ($s_size != '') {
                                echo pll_($s_size) . ' ' . pll_($unit);
                            } ?>
                        </div>
                    </div>
